...
class Array
  ...
  def bubble_sort(&prc)
    new_arr = self.dup.bubble_sort!(&prc)
  end
end
...
def substrings(string)
  ...
  (0...string.length).each do |first|
    (first...string.length).each do |second|
      substrings << string[first..second]
    end
  end
  substrings
end
...
def subwords(word, dictionary)
  ...
  substrings.select do |sub1|
    sub_words << sub1 if dictionary.include?(sub1)
  end
  sub_words
end
